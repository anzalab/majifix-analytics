...
const normalizeMetricTimes = data => {
  ...
  const times = lodash.map(keys, key => ({
    [key]: {
      minimum: normalizeTime(data[`minimum${lodash.upperFirst(key)}`]),
      maximum: normalizeTime(data[`maximum${lodash.upperFirst(key)}`]),
      average: normalizeTime(data[`average${lodash.upperFirst(key)}`]),
    },
  }));
...
const prepareReportResponse = results => {
  ...
  const data = lodash.merge(...results);

  if (!lodash.isEmpty(data.overall)) {
  ...
  if (data.jurisdictions) {
    data.jurisdictions = lodash.map(data.jurisdictions, normalizeMetricTimes);
  }

  if (data.priorities) {
    data.priorities = lodash.map(data.priorities, normalizeMetricTimes);
  }

  if (data.services) {
    data.services = lodash.map(data.services, normalizeMetricTimes);
  }

  if (data.groups) {
    data.groups = lodash.map(data.groups, normalizeMetricTimes);
  }
  ...
  if (data.zones) {
    data.zones = lodash.map(data.zones, normalizeMetricTimes);
  }

  if (data.assignees) {
    data.assignees = lodash.map(data.assignees, assignee => {
...
const getTimeFacet = metricTime => {
  const time = lodash.camelCase(metricTime);
  return {
    [`minimum${lodash.upperFirst(time)}`]: { $min: `$${time}` },
    [`maximum${lodash.upperFirst(time)}`]: { $max: `$${time}` },
    [`average${lodash.upperFirst(time)}`]: { $avg: `$${time}` },
  };
};
...
const getFacet = (facet, facetKeys) => {
  const newFacet = lodash.pick(facet, facetKeys);

  if (lodash.isEmpty(newFacet)) {
    return facet;
  }
...
const getChangelogBaseAggregation = criteria => {
  const ChangeLog = mongooseCommon.model('ChangeLog');
...
const getOperationalReport = (criteria, facetKeys, onResults) => {
  ...
  return async.parallel(
    ...
    (error, results) => {
      return onResults(error, lodash.flattenDeep(results));
    }
  );
};
